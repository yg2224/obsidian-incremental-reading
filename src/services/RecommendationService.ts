...
export class RecommendationService {
	...
	private async calculateSimilarityScores(
		...
		try {
			...
			for (const candidate of candidates) {
				...
				const priorityScore = SharedUtils.calculatePriority(metrics, this.settings.metricWeights) / 50; // Normalize to 0-1
			...
			return candidates.map(file => ({ file, score: Math.random() }));
		}
	}
	...
	private calculatePriorityScore(metrics: DocumentMetrics): number {
		return SharedUtils.calculatePriority(metrics, this.settings.metricWeights);
	}
}
